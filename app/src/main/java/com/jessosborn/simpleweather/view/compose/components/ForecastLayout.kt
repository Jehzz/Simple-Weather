...
@Composable
private fun forecastBorderColors(temps: Triple<WeatherSnapshot?, WeatherSnapshot?, WeatherSnapshot?>): List<Color> {
...
class ForecastPreviewParams : PreviewParameterProvider<ForecastWeather> {
	...
	private val timeValue =
		ArrayList<Long>().apply {
			add(currentTime)
			for (index in 1..23) add(currentTime + (10800 * index))
		}

	override val values: Sequence<ForecastWeather> =
		sequenceOf(
			ForecastWeather(
				listOf(
					WeatherSnapshot(
